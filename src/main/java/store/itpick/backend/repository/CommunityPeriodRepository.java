...
public interface CommunityPeriodRepository extends JpaRepository<CommunityPeriod, Long> {
    ...
    /** community가 'naver'이고 period가 'realtime'인 레코드 중에서 최근 업데이트된 10개를 조회하는 메서드 **/
    @Query("SELECT cp FROM CommunityPeriod cp WHERE cp.community = 'naver' AND cp.period = 'realtime' ORDER BY cp.communityPeriodId DESC")
    List<CommunityPeriod> findTop10ByNaverAndRealtime(Pageable pageable);
